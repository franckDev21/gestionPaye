    ...
    <div class="container">
        ...
        <div class="card">
            <div class="card-header">
                <div class="row align-items-center">
                    <div class="col-md-5">
                        <form id="form" style="transform: translateX(-10px)" action="#" class="form search">
                            <input autocomplete="off" placeholder="Rechercher un employé" type="texte" name="q"
                                class="inputSearch form-control">
                            <div class="result"></div>
                        </form>
                    </div>
                    <div class="col-md-4  p-4 parent__employer">
                        <span class="h5 text-center employe" style="color: #606060">Aucun Employé (veillez rechercher un
                            employé)</span>
                    </div>
                    <div class="col-md-3">
                ...
                <form action="/searchTransaction" method="POST" class="form2">
                    ...
                    <div style="z-index: 1" class="row mt-2 align-items-center bg bg-light-primary p-2">
                        ...
                        <div class="col-md-1">
                            <label for="transactionInt" style="color:var(--primary);font-weight:bold">Entrante</label>
                            <input type="checkbox" class="checkbox" style="height: 18px;width:18px" name="transactionInt"
                                id="transactionInt">
                        </div>
                        <div class="col-md-1">
                            <label for="transactionOut" style="color:var(--primary);font-weight:bold">Sortante</label>
                            <input type="checkbox" class="checkbox" style="height: 18px;width:18px" name="transactionOut"
                                id="transactionOut">
                        </div>
                        <div class="col-md-3">
                        ...
                        <div class="col-md-2">
                            ...
                            <div class="d-flex align-items-center">
                                <strong style="color:var(--primary)">Desc</strong>&nbsp;<input type="radio"
                                    style="height: 18px;width:18px" value="desc" name="trie" class="mr-1" id="">
                                &nbsp;&nbsp;&nbsp; <strong style="color:var(--primary)">Asc</strong>&nbsp;<input
                                    type="radio" style="height: 18px;width:18px" value="asc" name="trie" class="mr-1" id="">
                            </div>
                        </div>
                    </div>
                </form>
            ...
            <div class="card-body row">
                ...
                @else
                    @if (isset($all['transactionInt']) && empty($all['transactionInt']) && (isset($all['transactionOut']) && empty($all['transactionOut'])))
                        <div class="result__card noting p-4" style="width: 100%;">
                            <div class="result_noting p-4">Aucune transaction effectué</div>
                        </div>
                    @else
                        <table class="table table-head-custom table-responsive-sm">
                            ...
                            <tbody>
                                @if (count($all) > 1)
                                    @forelse ($all['transactionInt'] as $item)
                                    ...
                                    @empty
                                            <div class="result__card noting p-4" style="width: 100%;">
                                                <div class="result_noting p-4">Aucune transaction entrante</div>
                                            </div>
                                        @endforelse

                                        @forelse ($all['transactionOut'] as $item)
                                    ...
                                    @empty
                                            <div class="result__card noting p-4" style="width: 100%;">
                                                <div class="result_noting p-4">Aucune transaction sortante</div>
                                            </div>
                                        @endforelse
                                @else
                                        @isset($all['transactionInt'])
                                                @forelse ($all['transactionInt'] as $item)
                                                    <tr style="background-color: #06af1534">
                                                        <td>
                                                            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="20pt" height="20pt" viewBox="0 0 30 30" version="1.1">
                 ...
                 if (employerId || (dateDebut.value !== "" || dateFin.value !== "")) {
                     ...
                     if (dateDebut.value !== "" && dateFin.value !== "") {
                         ...
                         if (dateDebut > dateFin) {
                             ...
                             errorDiv.innerHTML =
                                 "<div class='alert alert-danger'>veuillez saisir une date correcte s’il vous plaît !</div>"
                             let id = setTimeout(() => {
                                 errorDiv.innerHTML = ''
                             }, 10000)
                         } else {
                     ...
                     if (transactionOut.checked == false && transactionInt.checked == false) {
                         let errorDiv = document.querySelector('.error_message')
                         errorDiv.innerHTML =
                             "<div class='alert alert-danger'>vous devez sélectionnez un employé ou une date avant de soumettre le formulaire</div>"
                         let id = setTimeout(() => {
                             errorDiv.innerHTML = ''
                         }, 10000)
                     } else {
             ...
             result.addEventListener('mouseenter', e => {
                 ...
                 if (card.length > 0) {
                     Array.from(card).forEach(elem => elem.addEventListener('click', e => {
                         ...
                         document.querySelector('.parent__employer').style.backgroundColor =
                             "#06af1560"
                         document.querySelector('.parent__employer').firstElementChild.style.color =
                             "#fff"
                         employeField.innerHTML =
                             `${data.querySelector('.result__name').innerHTML} <em>(${data.querySelector('.result__matricule').innerHTML})</em>`
